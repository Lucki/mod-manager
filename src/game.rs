...
impl Game {
    pub fn new(id: String, game_path: PathBuf) -> Result<Self, String> {
        ...
        let config_file = xdg_dirs_config.place_config_file(format!("{}.toml", id))
        ...
        std::fs::write(config_file, format!("path = \"{}\"", game_path
                .to_str()
    ...
    pub fn from_config(id: String, set_override: Option<String>) -> Result<Self, String> {
        ...
        let config_file = xdg_dirs_config.find_config_file(format!("{}.toml", id))
            .ok_or(format!("No config file found for game '{}'", id))?;

        let config = fs::read_to_string(&config_file)
            .or_else(|error| return Err(format!("Could not read config for game '{}': {}", id, error)))?
            .parse::<Value>()
            .or_else(|error| return Err(format!("Could not parse config for game '{}': {}", id, error)))?
            .as_table()
            .ok_or(format!("Could not parse config for game '{}': Root is not a table", id))?
            .to_owned();
        ...
        let writable = match config.get("writable") {
            Some(value) => value
                .as_bool()
                .ok_or(format!("Expected boolean for field 'writable' in game '{}'", id))?,
            None => false,
        };
        ...
        let should_run_pre_commands = match config.get("run_pre_commands") {
            Some(value) => value
                .as_bool()
                .ok_or(format!("Expected boolean for field 'run_pre_commands' in game '{}'", id))?,
            None => false,
        };
        ...
        let active_set = match set_override {
            ...
            None => {
                match config.get("active") {
                    Some(value) => {
                        let s = value
                            .as_str()
                            .ok_or(format!("Expected string for field 'active' in game '{}'", id))?
                            .to_owned();
                        match s.is_empty() {
                            true => None,
                            false => Some(s),
        ...
        let mod_tree = match &active_set {
            Some(a_set) => match config.get(a_set) {
                Some(value) => Some(ModSet::from_config(&a_set, value
                        .as_table()
                        .ok_or(format!("Expected table for field '{}' in game '{}'", &a_set, id))?,
                    id.clone(), &config, mod_root_path.clone(), &mut HashSet::new())?),
    ...
    pub fn activate(&self, writable: bool, is_setup: bool) -> Result<(), String> {
        ...
        if self.overlay.get_current_mounting_state()? == MountState::NORMAL {
            ...
            fs::rename(&self.path, &self.moved_path)
                .or_else(|e| Err(format!("Moving game dir for game '{}' failed: {}", &self.id, e)))?;
        }

        // Check if we're good to go, panic if not
        if self.overlay.get_current_mounting_state()? != MountState::MOVED {
            return Err(format!("Game '{}' is in an unexpected mounting state, aborting", &self.id));
        }
        ...
        if self.should_run_pre_commands() || (pre_commands.is_some() && !pre_commands.as_ref().unwrap().is_empty()) {
    ...
    pub fn deactivate(&self) -> Result<(), String> {
        ...
        for runtime_file in runtime_files {
            let pid = match runtime_file.file_stem() {
                Some(pid) => match pid.to_str() {
                    Some(pid) => match pid.parse::<i32>() {
                        Ok(pid) => pid,
                        Err(error) => {
                            println!("Failed parsing pid '{}' to i32 for game '{}': {}", pid, self.id, error);
                            continue;
                        },
                    },
                    None => {
                        println!("Failed converting OsStr '{:?}' to String for game '{}'", pid, self.id);
                        continue;
                    },
                },
                None => {
                    println!("Failed getting basename of file '{}' for game '{}'", runtime_file.display(), self.id);
                    continue;
                },
            };
            ...
            match kill_process(
                ...
                Signal::Term) {
                    Ok(_) => (),
                    Err(error) => {
                        println!("Terminating existing process '{}' for game '{}' failed: {}.", pid, self.id, error);
                        continue;
                    },
                }
            ...
            match fs::remove_file(&runtime_file) {
                Ok(_) => {},
                Err(error) => println!("Unable to remove PID file '{}' for game '{}': {}", runtime_file.display(), self.id, error),
            }
        }
        ...
        match self.overlay.get_current_mounting_state()? {
            ...
            MountState::MOUNTED => {
                match self.overlay.unmount() {
                    Ok(_) => {
                    },
                    Err(error) => return Err(format!("Unable to unmount game '{}': {}", self.id, error)),
                }
            },
            MountState::UNKNOWN => { return Err(format!("Unable to retrieve the current mounting state for game '{}'.", self.id)); },
            MountState::INVALID => { return Err(format!("Game '{}' is in an invalid mount state.", self.id)); },
            MountState::MOVED => { },
        }
        ...
        if self.overlay.get_current_mounting_state()? == MountState::MOVED {
            match fs::remove_dir(&self.path) {
                ...
                Err(e) => {
                    match e.kind() {
                        ErrorKind::NotFound => (),
                        _ => return Err(format!("Unable to remove the empty game '{}' directory '{}': {}.", self.id, self.path.display(), e)),
                    }
                }
            }

            fs::rename(&self.moved_path, &self.path)
                .or_else(|error| Err(format!("Unable to move the game '{}' back to it's original location: {}", self.id, error)))?;
        }

        self.overlay.change_cwd(true)
            .or_else(|error| return Err(format!("Could not change the current working directory for '{}' to '{}': {}", self.id, self.path.display(), error)))?;
    ...
    pub fn wrap(&self, command: ExternalCommand, writable: bool) -> Result<(), String> {
        ...
        match command.run() {
            Ok(_) => (),
            Err(error) => {
                println!("Unable to execute command for game '{}': {}", self.id, error);
            },
        }
    ...
    pub fn setup(&self, new_mod_id: String) -> Result<(), String> {
        ...
        let cache_path = self.xdg_dirs.create_cache_directory("persistent_setup")
            .or_else(|error| return Err(format!("Failed creating 'persistent_setup' for game '{}': {}", self.id, error)))?;

        if new_mod_path.is_dir() {
            return Err(format!("Mod '{}' already exists at '{}'", new_mod_id, new_mod_path.display()));
        }
    ...
    fn run_pre_commands(&self, mod_tree_commands: Option<Vec<&ExternalCommand>>) {
        ...
        if self.should_run_pre_commands() {
            match self.config.get("pre_command") {
                    ...
                    Some(array) => {
                        for (i, pre_command_table) in array.iter().enumerate() {
                            let pre_command_table = match pre_command_table.as_table() {
                                Some(table) => table,
                                None => {
                                    println!("Could not get 'pre_command' table for game '{}'", self.id);
                                    continue;
                                }
                            };

                            let pre_command = match ExternalCommand::from_config(self.id.clone(), i.to_string(), pre_command_table) {
                                Ok(c) => c,
                                Err(error) => {
                                    println!("Failed creating pre command '{}' for game '{}': {}", i.to_string(), error, self.id);
                                    continue;
                                },
                            };
                ...
                None => println!("'pre_command' expected to be defined for game '{}' but wasn't.", self.id),
            }
        }
        ...
        for command in commands {
            let process = match command.run() {
                Ok(p) => p,
                Err(error) => {
                    println!("Failed to run pre-command for game '{}': {}", self.id, error);
                    None
                },
            };
